...
type FractalStore = PersistedState & {
  ...
  createCalculator: (spaceId: ID, x: number, y: number) => ID;
  createMeasure: (spaceId: ID, x1: number, y1: number, x2: number, y2: number, unit: MeasureUnit) => ID | undefined;
  ...
  rotateObject: (id: ID, deltaDegrees: number, record?: boolean) => void;
...
export const useFractalStore = create<FractalStore>((set, get) => ({
  ...
  createCalculator: (spaceId, x, y) => {
    const id = makeId('calculator');
    const ts = now();
    const object: CalculatorFragment = { id, spaceId, x, y, width: 250, height: 360, type: 'calculator', createdAt: ts, updatedAt: ts };
    const before = makeSnapshotFrom(get());
    set((state) => ({ objects: { ...state.objects, [id]: object }, selectedIds: [id], ...withSnapshot(state, before, 'Pinned calculator') }));
    return id;
  },

  createMeasure: (spaceId, x1, y1, x2, y2, unit) => {
    ...
    const object: MeasureFragment = {
      id,
      spaceId,
      x: minX,
      y: minY,
      width: Math.max(68, maxX - minX),
      height: Math.max(68, maxY - minY),
      type: 'measure',
      unit,
    ...
    const before = makeSnapshotFrom(get());
    set((state) => ({ objects: { ...state.objects, [id]: object }, selectedIds: [id], ...withSnapshot(state, before, `Added measurement (${unit})`) }));
    return id;
  },
  ...
  rotateObject: (id, deltaDegrees, record = true) => {
    const object = get().objects[id];
    if (!object || object.type === 'bubble' || object.type === 'frame' || object.type === 'measure') return;
    const before = record ? makeSnapshotFrom(get()) : undefined;
    const next = (((object.rotation ?? 0) + deltaDegrees) % 360 + 360) % 360;
    set((state) => ({
      objects: { ...state.objects, [id]: { ...object, rotation: next, updatedAt: now() } as SpatialObject },
      revision: state.revision + 1,
      saveStatus: 'saving',
      ...(before ? withSnapshot(state, before, `Rotated object to ${Math.round(next)}°`) : {}),
    }));
  },
